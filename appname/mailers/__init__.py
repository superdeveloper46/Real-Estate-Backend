...
class Mailer:
    ...
    def send_by_mailgun(self, to_emails, subject, html_body, **kwargs):
        # proxies={
        #     'http': 'http://92.223.90.57:5432',
        #     'https': 'http://92.223.90.57:5432',
        # }
        url=f"""{os.getenv('MAILGUN_API_URL')}/{os.getenv('MAILGUN_DOMAIN')}/messages"""
        api_key=os.getenv('MAILGUN_API_KEY')
        payload={
            "from": "SFR Analytics <team@sfranalytics.com>",
            "to": to_emails,
            "subject": subject,
            "html": html_body
        }

        # if os.getenv('MAILGUN_PROXY_MODE'):
